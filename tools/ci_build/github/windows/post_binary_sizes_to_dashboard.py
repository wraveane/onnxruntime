...
def write_to_db(binary_size_data, args):
    ...
    fields = ["build_time", "onnxruntime", "build_id", "commit_id", "os", "arch", "build_config", "size"]
    now_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    rows = []
    for row in binary_size_data:
        rows.append([now_str, args.build_project, args.build_id, args.commit_hash,
                     row['os'], row['arch'], row['build_config'], row['size']])
